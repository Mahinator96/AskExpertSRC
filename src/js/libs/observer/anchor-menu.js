...
(function() {
	...
	const anchorMenuCollback = (entries) => { 																							 // Callback при триггере
		entries.forEach(entry => {																														 // Перебор секций
			...
			if (entry.isIntersecting && (entry.intersectionRatio >= sectionThrashold[0] || entry.intersectionRatio <= sectionThrashold[1]) ) { 												// Если секция видна и она видна больше чем на 50%									// Убрать у всех ссылок-якорей
